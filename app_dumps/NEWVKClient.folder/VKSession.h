...
@class NavigationStats, ViewPostStatsManager, MessagesPushSettings, NSArray, NotificationsManager, VKStats, NSURL, NSString, VKOnlineStore, VKClient, StoreStockManager, ExternalAdsStatsManager, CommonStatsManager, VKGroupState, VKMStore, StickersRecentModel, AdsStatsManager, ActivityModel, VKConnectivityState, VideoUploadManager, VKDB, NSNumber;
...
@interface VKSession : XXUnknownSuperclass <StatsManagerDelegate> {
	...
	MessagesPushSettings* _msgPushSettings;
	VKMStore* _store;
	VKClient* _client;
	VKStats* _stats;
...
@property(readonly, retain, nonatomic) NavigationStats* navigationStats;
...
@property(retain, nonatomic) MessagesPushSettings* msgPushSettings;
@property(readonly, assign, nonatomic) NSArray* domains;
+(id)storedSession;
+(void)store:(id)store;
...
-(void)changeMsgPushSettings:(id)settings;
...
-(BOOL)routeVKPrettyCards:(id)cards card:(id)card action:(id)action context:(id)context;
@end
